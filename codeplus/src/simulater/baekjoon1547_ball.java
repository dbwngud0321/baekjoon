...
public class baekjoon1547_ball {
	...
	public static void main(String[] args) {
		...
		M = scan.nextInt();
		int [] arr = {1,0,0}; //1번컵에 공이 있음
		int temp = 0;
		int a = 0;
		int b = 0;
		for(int i=0; i<M; i++) {
			temp = 0;
			a = scan.nextInt() - 1; // arr 배열이 index가 0부터 시작이므로 -1해준다
			b = scan.nextInt() - 1;
		...
		for(int i=0; i<arr.length; i++){
			if(arr[i] == 1) // 컵에 공이 있다면!
				System.out.println(i+1); // index가 0부터시작하므로 +1 해준다
